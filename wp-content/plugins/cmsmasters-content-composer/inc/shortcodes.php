...
public function cmsmasters_quote($atts, $content = null) {
	...
	'">' . "\n" . 
		...
		cmsmasters_composer_ob_load_template('theme-framework/postType/quote/quote-' . $this->quotes_atts['quote_mode'] . '.php', $this->quotes_atts) . 
		...
		if ($pinterest == 'true') {
			if (has_post_thumbnail()) {
				...
				if (!empty($img_matches[2][0])) {
					...
					$first_img = get_template_directory_uri() . '/theme-framework/img/logo.png';
				}
				...
				if ($tweets != '') {
					...
					echo '<div class="cmsmasters_notice cmsmasters_notice_error cmsmasters_theme_icon_cancel">' . "\n" . 
						'<div class="notice_content">' . "\n" . 
							'<p>' . esc_html__('Please add your Twitter API keys', 'cmsmasters-content-composer') . ', ' . '<a target="_blank" href="http://docs.cmsmasters.net/admin2/twitter-functionality/">' . esc_html__('read more how', 'cmsmasters-content-composer') . '</a></p>' . "\n" . 
						'</div>' . "\n" . 
					'</div>' . "\n";
				}
			...
			">";
				...
				if ($post_type == 'post') {
					$out .= '<div class="cmsmasters_owl_slider_item">';
						...
						while ($query->have_posts()) : $query->the_post();
							...
							$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/posts-slider/slider-post.php', $this->posts_slider_atts);
				...
				if ($post_type == 'project') {
					while ($query->have_posts()) : $query->the_post();
						...
						$out .= '<div class="cmsmasters_owl_slider_item">' . 
							cmsmasters_composer_ob_load_template('theme-framework/postType/posts-slider/slider-project.php', $this->posts_slider_atts) . 
						'</div>';
...
public function cmsmasters_blog($atts, $content = null) {
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'columns') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/blog/post-masonry.php', $this->blog_atts);
			} elseif ($layout == 'timeline') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/blog/post-timeline.php', $this->blog_atts);
			} else {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/blog/post-default.php', $this->blog_atts);
			}
		endwhile;
...
public function cmsmasters_portfolio($atts, $content = null) {
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'puzzle') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/portfolio/project-puzzle.php', $this->portfolio_atts);
			} else {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/portfolio/project-grid.php', $this->portfolio_atts);
			}
		endwhile;
...
public function cmsmasters_profiles($atts, $content = null) { 
	...
	if ($query->have_posts()) :
        ...
        while ($query->have_posts()) : $query->the_post();
			...
			if ($layout == 'vertical') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/profile/profile-vertical.php', $this->profiles_atts);
			} else {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/postType/profile/profile-horizontal.php', $this->profiles_atts);
			}
